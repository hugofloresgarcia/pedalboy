...
// 		("looper_init: server is " + this.server.asString).postln;
//
// 		if(server.isNil.not, {
...
//
// 	looperinit{
// 		("looperinit: server is " + this.server.asString).postln;
...
GrainLooper : PedalBoy {
	...
	*looper{|server, in, out, group|
	...
	init{|server, in, out, group|
		("INITING GRAINLOOPER").postln;
		("server: " + server.asString).postln;
		num_active = num_active + 1;
		this.instance_id = num_active;
		this.server = server;
		this.in = in;
		...
		("looper_init: server is " + this.server.asString).postln;
		("looper_init: in bus is " + this.in.asString).postln;
		("looper_init: out bus is " + this.out.asString).postln;
		("looper_init: target is " + this.group.asString).postln;
	...
	looperinit{
		// this.server  = this.server ? Server.default;
		if(this.server.isNil.not,{
			...
			("looperinit: server is " + this.server.asString).postln;
			("looperinit: target is " + this.group.asString).postln;
			this.set_mappable_args(this.mappable_args);
